...
public class CaptureHelper extends ScreenRecorder {
    ...
    public static void startRecord(String methodName) {
        ...
        File file = new File(SystemUtils.getCurrentDir() + Configuration.RECORDVIDEOS);
    ...
    public static void captureScreenshot(String screenshotName) {
        try {
            ...
            File theDir = new File(SystemUtils.getCurrentDir() + Configuration.SCREENSHOTS);
            if (!theDir.exists()) {
                theDir.mkdirs();
            }
            // Chổ này đặt tên thì truyền biến "screenName" gán cho tên File chụp màn hình
            FileHandler.copy(source, new File(SystemUtils.getCurrentDir() + Configuration.SCREENSHOTS + File.separator + screenshotName + "_" + dateFormat.format(new Date()) + ".png"));
